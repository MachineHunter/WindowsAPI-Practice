...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	QWORD base = (QWORD)GetModuleHandle("kernel32.dll");
	QWORD patch_addr = base + 0x368c0;
	DWORD curProtection, temp;
	int len = 13;

	VirtualProtect(patch_addr, len, PAGE_EXECUTE_READWRITE, &curProtection);
	memset(patch_addr, 0x90, len);
	VirtualProtect(patch_addr, len, curProtection, &temp);


	// write stolen code to code cave
	QWORD stolen_addr = (QWORD)0x402f48;
	int len2 = 25;

	VirtualProtect(patch_addr, len2, PAGE_EXECUTE_READWRITE, &curProtection);
	memset(patch_addr, 0x90, len2);
	...
	*(BYTE*)(stolen_addr + 24) = 0xe0;                    // jmp rax
	...
	VirtualProtect(patch_addr, len2, curProtection, &temp);
